...
var arg, argTab, build_assets, config, count, del, dirpath, dockerExec, dockerRun, e, each, exec, fs, http, i, ini, list, multimatch, name, parameters, params, path, printError, repo, rm, setPort, start, stop, sync, url, util, _i, _len,
...
build_assets = function(repo, config) {
  var buf, element, key, repopath, url_name, url_path, _results;
...
dockerExec = function(action, repo, callback) {
  ...
  return exec("if command -v boot2docker; then boot2docker up && $(boot2docker shellinit); fi\ndocker " + action + " repo_" + repo, callback);
};
...
switch (arg.command) {
  ...
  case 'sync':
    ...
    repo = (function() {
      ...
      _results = [];
      for (i = _j = 0, _len1 = repo.length; _j < _len1; i = ++_j) {
        name = repo[i];
        _results.push({
          name: name,
          url: url[i]
