...
var arg, argTab, buildAssetsFiles, config, count, del, dirpath, dockerExec, e, each, exec, fs, http, i, ini, list, multimatch, parameters, params, path, rm, setPort, start, stop, sync, syncRepo, url, util, _i, _len,
...
buildAssetsFiles = function(repo, u, config, next) {
  ...
  return fs.mkdir("" + repopath + "/assets", function() {
    ...
    return fs.writeFile("" + repopath + "/assets/init", buf, function() {
      return fs.chmod("" + repopath + "/assets/init", next);
    });
  });
};
    ...
    do_end = function() {
      return exec("docker run -v " + repopath + "/assets/:/app/ -v " + repopath + ":/var/ryba --rm=true --entrypoint /app/init ryba_repos/syncer", function(r_err, r_stdout, r_stderr) {
        if (r_err) {
          ...
          return dockerExec(repo, 'start', port);
        }
      });
    };
    if (exists) {
...
dockerExec = function(repo, action, port) {
  var do_run;
  if (action === 'start' && (port != null)) {
    do_run = function() {
      setPort(repo, port);
      return exec("docker run --name=repo_" + repo + " -d -v " + dirpath + "/" + repo + ":/usr/local/apache2/htdocs/ -p " + config[repo] + ":80 httpd");
    };
    if (config[repo] != null) {
      return exec("docker stop repo_" + repo + " && docker rm repo_" + repo, do_run);
    } else {
      return do_run();
    }
  } else {
...
start = function(repos, ports) {
  ...
  startEach = function(r) {
    return each(r).parallel(true).on('item', function(repo, index, next) {
      return dockerExec("repo_" + repo, 'start', ports != null ? ports[index] : void 0);
    }).on('both', function(err) {
      if (err) {
...
stop = function(repos) {
  ...
  stopEach = function(r) {
    return each(r).parallel(true).on('item', function(repo, index, next) {
      return dockerExec("repo_" + repo, 'stop');
    }).on('both', function(err) {
      if (err) {
        return console.log('stop finished with errors');
      } else {
...
del = function(repos) {
  return each(repos).parallel(true).on('item', function(repo, next) {
    return exec("docker rm repo_" + repo, function(err, stdout, stderr) {
